...
class ClientController extends Controller
{
   ...
   {
       $response = Http::get("http://127.0.0.1:8000/api/menu");
       $data = json_decode($response);
       $products = $data->data->product;
   ...
   {
       $response = Http::get("http://127.0.0.1:8000/api/menu/".$request->id);
       $data = json_decode($response);
       $products = $data->data->product;
       $topping = $data->data->topping;
   ...
   {
       $response = Http::post("http://127.0.0.1:8000/api/postsignUp",[
           'name'=>$request->name,
           'email'=>$request->email,
   ...
   {
       $response = Http::get("http://127.0.0.1:8000/api/search/".$request->key);
       $data = json_decode($response);
   ...
   public function qlKH(Request $request){
       $id = Auth::user()->id;
       $response = Http::get("http://127.0.0.1:8000/api/qltk/".$id);
       $data = json_decode($response);
       $customer = $data->data->customer;
       $user = $data->data->user;
   ...
   public function changePassWord(Request $request){
       ...
       $response = Http::post("http://127.0.0.1:8000/api/changePassword",$data);
       return back()->with('thong bao','Succeedful!');
   }

   public function changeCus(Request $request){
       $user_id = Auth::user()->id;
       $response = Http::post("http://127.0.0.1:8000/api/changeCus",[
           'address'=>$request->address,
           'phone'=>$request->phone,
           'user_id'=>$user_id
       ]);
   ...
   public function comment(Request $request){
       $response = Http::post("http://127.0.0.1:8000/api/comment",[
